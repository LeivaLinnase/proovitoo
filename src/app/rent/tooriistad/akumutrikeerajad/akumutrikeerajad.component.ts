...
@Component({
  ...
  templateUrl: './akumutrikeerajad.component.html',
  styleUrls: ['./akumutrikeerajad.component.css']
})
export class AkumutrikeerajadComponent implements OnInit {
  akumutrikeerajad: {id: number, name: string, price: number, description: string, category: string, imgSrc: string, isActive: boolean}[] = [];
  products: Product[] = [];
  ...
  ngOnInit(): void {
    ...
    this.http.get<{id: number, name: string, price: number, description: string, category: string, imgSrc: string, isActive: boolean}[]>(this.dbUrl).subscribe(productsFromDb => { 
      ...
      this.akumutrikeerajad = newArray;
    }) ;
  }
