...
@Component({
  ...
  templateUrl: './trellikomplektid.component.html',
  styleUrls: ['./trellikomplektid.component.css']
})
export class TrellikomplektidComponent implements OnInit {
  trellikomplektid: {id: number, name: string, price: number, description: string, category: string, imgSrc: string, isActive: boolean}[] = [];
  products: Product[] = [];
  ...
  ngOnInit(): void {
    ...
    this.http.get<{id: number, name: string, price: number, description: string, category: string, imgSrc: string, isActive: boolean}[]>(this.dbUrl).subscribe(productsFromDb => { 
      ...
      this.trellikomplektid = newArray;
    }) ;
  }
